    ...
    methods
        ...
        function d = geostep(obj, idz, knew, kopt, bmat, delbar, xpt, zmat)
            ...
            if denrat <= 0.8
                ...
                if abs(alpha * beta + fortran.power(vlag(knew), 2)) >= abs(denom) ...
                   || infnan_obj.is_nan_sp(denom)
                    d = dden;
                end
            end
        ...
        function d = biglag(obj, idz, knew, bmat, delbar, x, xpt, zmat)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(linalg_obj.issymmetric(bmat(:, npt + 1:npt + n)), ...
                                 "BMAT(:, NPT+1:NPT+N) is symmetric", srname);
                debug_obj.assert(delbar > 0, "DELBAR > 0", srname);
                debug_obj.assert(numel(x) == n && all(infnan_obj.is_finite(x), 'all'), ...
                                 "SIZE(X) == N, X is finite", srname);
                debug_obj.assert(all(infnan_obj.is_finite(xpt), 'all'), "XPT is finite", srname);
                debug_obj.assert(size(zmat, 1) == npt && size(zmat, 2) == npt - n - 1, ...
                                 "SIZE(ZMAT) == [NPT, NPT - N - 1]", srname);
            end
            ...
            for iter = 1:maxiter
                ...
                if abs(obj.circle_fun_biglag(angle, cf)) ...
                   <= 1.1 * abs(obj.circle_fun_biglag(consts_obj.ZERO, cf))
                    break
                end
        ...
        function d = bigden(obj, idz, knew, kopt, bmat, d0, xpt, zmat)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(linalg_obj.issymmetric(bmat(:, npt + 1:npt + n)), ...
                                 "BMAT(:, NPT+1:NPT+N) is symmetric", srname);
                debug_obj.assert(numel(d0) == n && all(infnan_obj.is_finite(d0), 'all'), ...
                                 "SIZE(D0) == N, D0 is finite", srname);
                debug_obj.assert(all(infnan_obj.is_finite(xpt), 'all'), "XPT is finite", srname);
                debug_obj.assert(size(zmat, 1) == npt && size(zmat, 2) == npt - n - 1, ...
                                 "SIZE(ZMAT) == [NPT, NPT - N - 1]", srname);
            end
            ...
            if ~(fortran.power(ds, 2) <= 0.99 * dd * ss)
                ...
                sstemp(kopt) = ss;
                k = fortran.minloc(fortran.power(dstemp, 2) ./ sstemp, 'dim', 1);
            ...
            for iter = 1:n
                ...
                for k = 1:npt
                    ...
                    w(k, 1) = ...
                        consts_obj.QUART * (fortran.power(tempa, 2) + fortran.power(tempb, 2));
                    w(k, 2) = tempa * tempc;
                    w(k, 3) = tempb * tempc;
                    w(k, 4) = ...
                        consts_obj.QUART * (fortran.power(tempa, 2) - fortran.power(tempb, 2));
                ...
                for j = 1:5
                    ...
                    if j == 2 || j == 3
                        prod_custom(1:npt, j) = ...
                            prod_custom(1:npt, j) ...
                            + linalg_obj.matprod12(w(npt + 1:npt + n, j), bmat(:, 1:npt));
                        nw = npt + n;
                    end
                    prod_custom(npt + 1:npt + n, j) = ...
                        linalg_obj.matprod21(bmat(:, 1:nw), w(1:nw, j));
                end
                ...
                par(:) = ...
                    [consts_obj.ONE, fortran.cos(angle), fortran.sin(angle), ...
                     fortran.cos(2.0 * angle), fortran.sin(2.0 * angle)];
