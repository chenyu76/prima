classdef univar_mod
    ...
    methods
        function angle = circle_min(~, fun, args, grid_size)
            ...
            agrid = ...
                linalg_obj.linspace_r(consts_obj.ZERO, consts_obj.TWO * consts_obj.PI, ...
                                      grid_size ...
                                      + 1); % Size: GRID_SIZE+1; the last entry will be unused
        ...
        function angle = circle_maxabs(~, fun, args, grid_size)
            ...
            agrid = ...
                linalg_obj.linspace_r(consts_obj.ZERO, consts_obj.TWO * consts_obj.PI, ...
                                      grid_size ...
                                      + 1); % Size: GRID_SIZE+1; the last entry is not used
            fgrid = arrayfun(@(k) fun(agrid(k), args), (1:grid_size)');
            %%MATLAB: fgrid = arrayfun(@(angle) fun(angle, args), agrid(1:grid_size));  % Same shape as `agrid`
        ...
        function x = interval_max(~, fun, lb, ub, args, grid_size)
            ...
            % Preconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(infnan_obj.is_finite(lb) && infnan_obj.is_finite(ub) ...
                                 && lb <= ub, "LB <= UB and they are finite", srname);
                debug_obj.assert(grid_size >= 3, "GRID_SIZE >= 3", srname);
            end
