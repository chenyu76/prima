classdef linalg_mod
    ...
    methods
        ...
        function varargout = matprod(obj, varargin)
            if numel(varargin) == 2 && isvector(varargin{1}) ...
               && (~isvector(varargin{2}) && ~isscalar(varargin{2}))
                [varargout{1:nargout}] = obj.matprod12(varargin{:});
            elseif numel(varargin) == 2 && (~isvector(varargin{1}) && ~isscalar(varargin{1})) ...
                   && isvector(varargin{2})
        ...
        function varargout = sort(obj, varargin)
            if numel(varargin) >= 1 && numel(varargin) <= 2 && isvector(varargin{1}) ...
               && (numel(varargin) < 2 || ischar(varargin{2}))
        ...
        function varargout = norm(obj, varargin)
            if numel(varargin) >= 1 && numel(varargin) <= 2 && isvector(varargin{1}) ...
               && (numel(varargin) < 2 || isfloat(varargin{2}))
        ...
        function A = r1_sym(obj, A, alpha, x)
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(A, 1) == n && obj.issymmetric(A), ...
                                 "A is N-by-N and symmetric", srname);
            end
        end
        function A = r1(obj, A, alpha, x, y)
            ...
            % Preconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(A, 1) == numel(x) && size(A, 2) == numel(y), ...
                                 "SIZE(A) == [SIZE(X), SIZE(Y)]", srname);
            end
        ...
        function z = matprod22(~, x, y)
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(z, 1) == size(x, 1) && size(z, 2) == size(y, 2), ...
                                 "[SIZE(Z) == SIZE(X, 1), SIZE(Y, 2)]", srname);
            end
        end
        ...
        function z = outprod(~, x, y)
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(z, 1) == numel(x) && size(z, 2) == numel(y), ...
                                 "SIZE(Z) == [SIZE(X), SIZE(Y)]", srname);
            end
        end
        function x = eye1(~, n)
            %--------------------------------------------------------------------------------------------------%
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(x, 1) == max(n, 0) && size(x, 2) == max(n, 0), ...
                                 "SIZE(X) == [N, N]", srname);
            end
        end
        function x = eye2(~, m, n)
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(x, 1) == max(m, 0) && size(x, 2) == max(n, 0), ...
                                 "SIZE(X) == [M, N]", srname);
            end
        end
        function x = solve(obj, A, b)
            ...
            if obj.istril(A)
                for i = 1:n
                    x(i) = ...
                        (b(i) - obj.inprod(A(i, 1:i - 1).', x(1:i - 1))) ...
                        / A(i, i); % INPROD = 0 if I == 1.
                end
            elseif obj.istriu(A)
                % This case is invoked in LINCOA.
                for i = n:-1:1
                    x(i) = ...
                        (b(i) - obj.inprod(A(i, i + 1:n).', x(i + 1:n))) ...
                        / A(i, i); % INPROD = 0 if I == N.
                end
            else
                % This is NOT a good algorithm for linear systems, but since the QR subroutine is available ...
                [Q, R, P] = obj.qr(A, 'Q', Q, 'R', R, 'P', P);
                x(:) = obj.matprod12(b, Q);
                for i = n:-1:1
                    x(i) = ...
                        (x(i) - obj.inprod(R(i, i + 1:n).', x(i + 1:n))) ...
                        / R(i, i); % INPROD = 0 if I == N.
                end
            ...
            if consts_obj.DEBUGGING
                ...
                if infnan_obj.is_finite(fortran.sum(abs(A), 'all') + fortran.sum(abs(b), 'all'))
                    tol = ...
                        max(fortran.power(consts_obj.TEN, max(-8, -consts_obj.MAXPOW10)), ...
                            min(0.1, ...
                                fortran.power(consts_obj.TEN, min(8, consts_obj.MAXPOW10)) ...
                                * consts_obj.EPS * double(n + 1)));
                    debug_obj.assert(obj.p_norm(obj.matprod21(A, x) - b) ...
                                     <= tol ...
                                        * max([consts_obj.ONE, obj.p_norm(b), obj.p_norm(x)]), ...
                                     "A*X == B", srname);
                end
            end
        end
        function B = inv(obj, A)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(obj.istril(B) || ~obj.istril(A), ...
                                 "If A is lower triangular, then so is B", srname);
                debug_obj.assert(obj.istriu(B) || ~obj.istriu(A), ...
                                 "If A is upper triangular, then so is B", srname);
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-8, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                            fortran.power(consts_obj.TEN, min(10, consts_obj.MAXPOW10)) ...
                            * consts_obj.EPS * double(n + 1)));
        ...
        function is_inv = isinv(obj, A, B, varargin)
            ...
            is_inv = ...
                all(abs(obj.matprod22(A, B) - obj.eye1(n)) <= tol_loc, 'all') ...
                || all(abs(obj.matprod22(B, A) - obj.eye1(n)) <= tol_loc, 'all');
        ...
        function [Q, R, P] = qr(obj, A, varargin)
            ...
            for j = 1:n
                if pivot
                    k = fortran.maxloc(fortran.sum(fortran.power(T(j:n, j:m), 2), 2), 'dim', 1);
                    if k > 1 && k <= n - j + 1
                        k = k + j - 1;
                        P([j, k]) = P([k, j]);
                        T([j, k], :) = T([k, j], :);
                ...
                for i = m:-1:j + 1
                    ...
                    T(j, [j, i]) = ...
                        [obj.hypotenuse(T(j, j), T(j, i)), ...
                         consts_obj.ZERO]; %T(j, [j, i]) = [sqrt(T(j, j)**2 + T(j, i)**2), ZERO]
            ...
            if consts_obj.DEBUGGING
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-10, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                            fortran.power(consts_obj.TEN, min(4, consts_obj.MAXPOW10)) ...
                            * consts_obj.EPS * double(max(m, n) + 1)));
                debug_obj.assert(obj.isorth(Q_loc, 'tol', tol), ...
                                 "The columns of Q are orthonormal", srname);
                debug_obj.assert(obj.istril(T, 'tol', tol), "R is upper triangular", srname);
                if pivot
                    debug_obj.assert(all(abs(obj.matprod22(Q_loc, T.') - A(:, P)) ...
                                         <= max(tol, tol * max(abs(A), [], 'all')), 'all'), ...
                                     "A(:, P) == Q*R", srname);
                    for j = 1:min(m, n) - 1
                        ...
                        debug_obj.assert(all(fortran.power(T(j, j), 2) ...
                                             + max(tol, tol * fortran.power(T(j, j), 2)) ...
                                             >= fortran.sum(fortran.power(T(j + 1:n, ...
                                                                            j:min(m, n)), 2), ...
                                                            2), 'all'), ...
                                         "R(J, J)^2 >= SUM(R(J : MIN(M, N), J + 1 : N).^2", srname);
                    end
                else
                    debug_obj.assert(all(abs(obj.matprod22(Q_loc, T.') - A) ...
                                         <= max(tol, tol * max(abs(A), [], 'all')), 'all'), ...
                                     "A == Q*R", srname);
                end
            end
            ...
            if consts_obj.DEBUGGING
                ...
                if ~ismember('Q', ipObj.UsingDefaults)
                    ...
                    tol = ...
                        max(fortran.power(consts_obj.TEN, max(-10, -consts_obj.MAXPOW10)), ...
                            min(0.1, ...
                                fortran.power(consts_obj.TEN, min(6, consts_obj.MAXPOW10)) ...
                                * consts_obj.EPS * double(max(m, n) + 1)));
                    debug_obj.assert(obj.isorth(Q, 'tol', tol), ...
                                     "The columns of Q are orthogonal", srname);
                end
                if ~ismember('Rdiag', ipObj.UsingDefaults)
                    debug_obj.assert(numel(Rdiag) == min(m, n), "SIZE(R) == MIN(M, N)", srname);
                    debug_obj.assert(~ismember('Q', ipObj.UsingDefaults), ...
                                     "Rdiag is present only if Q is present", srname);
                end
            end
            ...
            if consts_obj.DEBUGGING
                ...
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-10, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                            fortran.power(consts_obj.TEN, min(6, consts_obj.MAXPOW10)) ...
                            * consts_obj.EPS * double(m + 1)));
                debug_obj.assert(obj.isorth(Q, 'tol', tol), "The columns of Q are orthogonal", ...
                                 srname);
        ...
        function D = diag(~, A, varargin)
            ...
            memory_obj.alloc_rvector_sp(dlen);
            if k_loc >= 0
        ...
        function is_banded = isbanded(~, A, lwidth, uwidth, varargin)
            ...
            tol = ipObj.Results.tol;
            if consts_obj.DEBUGGING
                debug_obj.assert(lwidth >= 0 && uwidth >= 0, "LWIDTH >= 0 .and. UWIDTH >= 0", ...
                                 srname);
                if ~ismember('tol', ipObj.UsingDefaults)
                    debug_obj.assert(tol >= 0, "TOL >= 0", srname);
                end
        ...
        function is_orth = isorth(obj, A, varargin)
            ...
            elseif consts_obj.ORTHTOL_DFT < consts_obj.REALMAX
                is_orth = ...
                    all(abs(obj.matprod22(A.', A) - obj.eye1(n)) ...
                        <= max(tol_loc, tol_loc * max(abs(A), [], 'all')), 'all');
            end
        ...
        function y = project1(obj, x, v)
            ...
            elseif any(infnan_obj.is_inf(v), 'all')
                u(:) = consts_obj.ZERO;
                u(obj.trueloc(infnan_obj.is_inf(v))) = ...
                    fortran.sign(consts_obj.ONE, v(obj.trueloc(infnan_obj.is_inf(v))));
                    ...
                    tol = ...
                        max(fortran.power(consts_obj.TEN, max(-10, -consts_obj.MAXPOW10)), ...
                            min(0.1, ...
                                fortran.power(consts_obj.TEN, min(6, consts_obj.MAXPOW10)) ...
                                * consts_obj.EPS));
                    debug_obj.assert(obj.p_norm(y) <= (consts_obj.ONE + tol) * obj.p_norm(x), ...
                                     "NORM(Y) <= NORM(X)", srname);
                    debug_obj.assert(obj.p_norm(x - y) ...
                                     <= (consts_obj.ONE + tol) * obj.p_norm(x), ...
                                     "NORM(X - Y) <= NORM(X)", srname);
                    % The following test may not be passed.
                    debug_obj.assert(abs(obj.inprod(x - y, v)) ...
                                     <= max(tol, ...
                                            tol ...
                                            * max(obj.p_norm(x - y) * obj.p_norm(v), ...
                                                  abs(obj.inprod(x, v)))), ...
        ...
        function y = project2(obj, x, V)
            ...
            elseif any(infnan_obj.is_inf(V), 'all')
                mask00 = infnan_obj.is_inf(V);
                mask01 = ~mask00;
                V_loc(mask00) = fortran.sign(consts_obj.ONE, V(mask00));
                V_loc(mask01) = consts_obj.ZERO;
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                if infnan_obj.is_finite(obj.p_norm(x)) ...
                   && infnan_obj.is_finite(fortran.sum(fortran.power(V, 2), 'all'))
                    tol = ...
                        max(fortran.power(consts_obj.TEN, max(-10, -consts_obj.MAXPOW10)), ...
                            min(0.1, ...
                                fortran.power(consts_obj.TEN, min(6, consts_obj.MAXPOW10)) ...
                    ...
                    debug_obj.assert(obj.p_norm(x - y) ...
                                     ...
                                     "NORM(X - Y) <= NORM(X)", srname);
                    % The following test may not be passed.
                    debug_obj.assert(obj.p_norm(obj.matprod12(x - y, V)) ...
                                     <= max(tol, ...
                                            tol ...
                                            * max(obj.p_norm(x - y) ...
                                                  * obj.named_norm_mat(V, "fro"), ...
                                                  obj.p_norm(obj.matprod12(x, V)))), ...
                                     "X - Y is orthogonal to V", srname);
                end
            end
        end
        function r = hypotenuse(~, x1, x2)
            ...
            elseif ~infnan_obj.is_finite(x2)
                ...
                y(:) = abs([x1, x2]);
                y(:) = [min(y), max(y)];
                if y(1) > fortran.sqrt(consts_obj.REALMIN) ...
                   && y(2) < fortran.sqrt(consts_obj.REALMAX / 2.1)
            ...
            if consts_obj.DEBUGGING
                if infnan_obj.is_nan_sp(x1) || infnan_obj.is_nan_sp(x2)
                    debug_obj.assert(infnan_obj.is_nan_sp(r), "R is NaN if X1 or X2 is NaN", ...
                                     srname);
                else
                    debug_obj.assert(r >= abs(x1) && r >= abs(x2) && r <= abs(x1) + abs(x2), ...
        ...
        function G = planerot(obj, x)
            ...
            elseif abs(x(1)) <= consts_obj.EPS * abs(x(2))
                ...
                if all(abs(x) > fortran.sqrt(consts_obj.REALMIN) ...
                       & abs(x) < fortran.sqrt(consts_obj.REALMAX / 2.1), 'all')
                ...
                elseif abs(x(1)) > abs(x(2))
                    t = x(2) / x(1);
                    u = ...
                        max([consts_obj.ONE, abs(t), ...
                             fortran.sqrt(consts_obj.ONE ...
                                          + fortran.power(t, ...
                                                          2))]); % MAXVAL: precaution against rounding error.
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(abs(G(1, 1) - G(2, 2)) + abs(G(1, 2) + G(2, 1)) <= 0, ...
                                 "G(1,1) == G(2,2), G(1,2) = -G(2,1)", srname);
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-10, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                            fortran.power(10.0, min(6, consts_obj.MAXPOW10)) * consts_obj.EPS));
                debug_obj.assert(obj.isorth(G, 'tol', tol), "G is orthonormal", srname);
                if all(infnan_obj.is_finite(x) ...
                       & abs(x) < fortran.sqrt(consts_obj.REALMAX / 2.1), 'all')
                    r = obj.p_norm(x);
                    debug_obj.assert(max(abs(obj.matprod21(G, x) - [r, consts_obj.ZERO].')) ...
                                     <= max(tol, tol * r), "G * X = [||X||, 0]", srname);
                end
            end
        end
        ...
        function is_symmetric = issymmetric(~, A, varargin)
            ...
            elseif consts_obj.SYMTOL_DFT < 0.9 * consts_obj.REALMAX
                is_symmetric = ...
                    ~any(abs(A - A.') > tol_loc * max(max(abs(A), [], 'all'), consts_obj.ONE), ...
                         'all') && all(infnan_obj.is_nan_sp(A) == infnan_obj.is_nan_sp(A.'), 'all');
            end
        ...
        function y = p_norm(~, x, varargin)
            ...
            elseif maxabs <= 0
                ...
                elseif abs(p_loc - consts_obj.TWO) <= 0
                    ...
                    if infnan_obj.is_posinf(y) || y <= 0
                        scalmin = ...
                            fortran.power(double(radix(consts_obj.ZERO)), ...
                                          max(minexponent(consts_obj.ZERO) - 1, ...
                                              1 - maxexponent(consts_obj.ZERO)));
                        scalmax = ...
                            fortran.power(double(radix(consts_obj.ZERO)), ...
                                          min(maxexponent(consts_obj.ZERO) - 1, ...
                                              1 - minexponent(consts_obj.ZERO)));
                        scaling = min(max(maxabs, scalmin), scalmax);
                        y = ...
                            scaling ...
                            * fortran.sqrt(fortran.sum(fortran.power(x ./ scaling, 2), 'all'));
                    end
                else
                    y = ...
                        fortran.power(fortran.sum(fortran.power(abs(x), p_loc), 'all'), ...
                                      consts_obj.ONE / p_loc);
                    % The following code handles over/underflow naively.
                    if infnan_obj.is_posinf(y) || y <= 0
                        scalmin = ...
                            fortran.power(double(radix(consts_obj.ZERO)), ...
                        ...
                        y = ...
                            scaling ...
                            * fortran.power(fortran.sum(fortran.power(abs(x ./ scaling), p_loc), ...
                                                        'all'), consts_obj.ONE / p_loc);
                    end
                end
            end
            ...
            if consts_obj.DEBUGGING
                debug_obj.assert(y >= 0 || any(infnan_obj.is_nan_sp(x), 'all'), ...
                                 "Y >= 0 unless X contains NaN", srname);
                debug_obj.assert(infnan_obj.is_nan_sp(y) == any(infnan_obj.is_nan_sp(x), 'all'), ...
                                 "Y is NaN if and only if X contains NaN", srname);
                % Even with scaling, Y may still be 0 if all entries of X are zero or subnormal.
                debug_obj.assert(y > 0 || any(infnan_obj.is_nan_sp(x), 'all') ...
                                 || all(abs(x) < consts_obj.REALMIN, 'all'), ...
                                 "Y > 0 unless X contains NaN or all its entries are below REALMIN", ...
                                 srname);
            end
        ...
        function y = named_norm_vec(obj, x, nname)
            ...
            elseif ~any(abs(x) > 0, 'all')
                ...
                otherwise
                    debug_obj.warning(srname, ...
                                      "Unknown name of norm: " + string_obj.strip(nname) ...
                                      + "; default to the L2-norm");
                    y = obj.p_norm(x); % 2-norm, which is the default case of P_NORM.
                end
            end
        ...
        function y = named_norm_mat(~, x, nname)
            ...
            elseif ~any(abs(x) > 0, 'all')
                ...
                case "inf"
                    ...
                    % a 0 for robustness.
                    y = max([fortran.sum(abs(x), 2); consts_obj.ZERO], [], 'all');
                otherwise
                    debug_obj.warning(srname, ...
                                      "Unknown name of norm: " + string_obj.strip(nname) ...
            ...
            if consts_obj.DEBUGGING
                if dim_loc == 1
                    ...
                    if direction_loc == "ascend" || direction_loc == "ASCEND"
                        debug_obj.assert(all(y(1:n - 1, :) <= y(2:n, :), 'all'), ...
                                         "Y is ascending along dimension 1", srname);
                    else
                        debug_obj.assert(all(y(1:n - 1, :) >= y(2:n, :), 'all'), ...
                                         "Y is descending along dimension 1", srname);
                    end
                else
                    n = size(y, 2);
                    if direction_loc == "ascend" || direction_loc == "ASCEND"
                        debug_obj.assert(all(y(:, 1:n - 1) <= y(:, 2:n), 'all'), ...
                                         "Y is ascending along dimension 2", srname);
                    else
                        debug_obj.assert(all(y(:, 1:n - 1) >= y(:, 2:n), 'all'), ...
                                         "Y is descending along dimension 2", srname);
                    end
                end
            end
        end
        ...
        function loc = trueloc(obj, x)
            ...
            % Outputs
            % INTEGER(IK) :: LOC(COUNT(X)) does not work with Absoft 22.0
            % Local variables
            srname = "TRUELOC";
            ...
            memory_obj.alloc_ivector(nnz(x)); % Removable in F03.
            n = numel(x);
            loc = ...
                feval(@(source_array, ...
                        selection_mask) reshape(source_array(selection_mask ...
                                                             & true(size(source_array))), [], ...
                                                1), obj.linspace_i(1, n, n), x);
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(all(loc(2:numel(loc)) > loc(1:numel(loc) - 1), 'all'), ...
                                 "LOC is strictly ascending", srname);
            end
        end
        function loc = falseloc(obj, x)
            ...
            % Outputs
            % INTEGER(IK) :: LOC(COUNT(.NOT.X)) does not work with Absoft 22.0
            % Local variables
            srname = "FALSELOC";
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(numel(loc) == numel(x) - nnz(x), ...
                                 "SIZE(LOC) == SIZE(X) - COUNT(X)", srname);
        ...
        function y = minimum1(~, x)
            ...
            nan_test = ...
                fortran.sum(abs(x), ...
                            'all'); % 1. Assume: X has NaN iff NAN_TEST = NaN. 2. Avoid enormous calls to IS_NAN
            y = ...
                fortran.merge('tsource', nan_test, 'fsource', min(x), ...
                              'mask', infnan_obj.is_nan_sp(nan_test));
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(~infnan_obj.is_nan_sp(y) ...
                                 || any(infnan_obj.is_nan_sp(x), 'all'), ...
                                 "Y is not NaN unless X contains NaN", srname);
                debug_obj.assert(infnan_obj.is_nan_sp(y) ...
                                 || ~any(infnan_obj.is_nan_sp(x), 'all'), ...
                                 "Y is NaN if X contains NaN", srname);
            end
        end
        function y = minimum2(~, x)
            ...
            nan_test = ...
                fortran.sum(abs(x), ...
                            'all'); % 1. Assume: X has NaN iff NAN_TEST = NaN. 2. Avoid enormous calls to IS_NAN
            y = ...
                fortran.merge('tsource', nan_test, 'fsource', min(x, [], 'all'), ...
                              'mask', infnan_obj.is_nan_sp(nan_test));
        ...
        function y = maximum1(~, x)
            ...
            nan_test = ...
                fortran.sum(abs(x), ...
                            'all'); % 1. Assume: X has NaN iff NAN_TEST = NaN. 2. Avoid enormous calls to IS_NAN
            y = ...
                fortran.merge('tsource', nan_test, 'fsource', max(x), ...
                              'mask', infnan_obj.is_nan_sp(nan_test));
        ...
        function y = maximum2(~, x)
            ...
            nan_test = ...
                fortran.sum(abs(x), ...
                            'all'); % 1. Assume: X has NaN iff NAN_TEST = NaN. 2. Avoid enormous calls to IS_NAN
            y = ...
                fortran.merge('tsource', nan_test, 'fsource', max(x, [], 'all'), ...
                              'mask', infnan_obj.is_nan_sp(nan_test));
        ...
        function x = linspace_i(obj, xstart, xstop, n)
            ...
            x = ...
                round(obj.linspace_r(double(xstart), double(xstop), ...
                                     n)); % Rounded to the closest integer.
        ...
        function [A, tdiag, tsubdiag] = hessenberg_hhd_trid(obj, A, tdiag, tsubdiag)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(numel(tsubdiag) == max(0, n - 1), "SIZE(TDIAG) == MAX(0, N-1)", ...
                                 srname);
            end
            ...
            for k = 1:n - 1
                ...
                w(k + 1:n) = ...
                    fortran.sqrt(consts_obj.TWO / (colsq + fortran.power(A(k + 1, k), 2))) ...
                    * A(k + 1:n, k);
                %----------------------------------------------------------------------------------------------%
                ...
                tdiag(k + 1:n) = ...
                    tdiag(k + 1:n) + w(k + 1:n) .* (wz * w(k + 1:n) - consts_obj.TWO * z(k + 1:n));
                for j = k + 1:n
                    A(j + 1:n, j) = ...
        ...
        function [H, Q] = hessenberg_full(obj, A, H, varargin)
            ...
            for j = 1:n - 1
                ...
                v(j + 1:n) = ...
                    fortran.sqrt(consts_obj.TWO / (colsq + fortran.power(v(j + 1), 2))) ...
                    * v(j + 1:n);
                % The two lines above are from Powell. They are equivalent to the following two lines.
                % %V(J + 1) = V(J + 1) - SUBD
                ...
                for i = j + 1:n
                    H(j + 1:n, i) = ...
                        H(j + 1:n, i) - obj.inprod(H(j + 1:n, i), v(j + 1:n)) * v(j + 1:n);
                end
                H(j + 1, j) = subd;
            ...
            if consts_obj.DEBUGGING
                ...
                if nargout >= 2 || ~ismember('Q', ipObj.UsingDefaults)
                    ...
                    debug_obj.assert(all(abs(obj.matprod22(Q, H) - obj.matprod22(A, Q)) ...
                                         <= tol * max(abs(A), [], 'all'), 'all'), "Q*H = A*Q", ...
                                     srname);
                end
            end
        end
        function eig_min = eigmin_sym_trid(obj, td, tn, varargin)
            ...
            if all(piv >= 0, 'all')
                ...
                eminub = min(td);
                eminlb = -max(abs([consts_obj.ZERO; tn]) + abs(td) + abs([tn; consts_obj.ZERO]));
            end

            ksav = 0;
            pivksv = ...
                consts_obj.ZERO; % This initial value will not be used, but Fortran compilers may complain without it.
        ...
        function y = smat_mul_vec(obj, smatv, x)
            ...
            % Preconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(numel(smatv) == n * (n + 1) / 2, "SIZE(SMATV) = N*(N+1)/2", ...
                                 srname);
            end
