    ...
    methods
        ...
        function [Q, Rdiag, n] = qradd_Rdiag(~, c, Q, Rdiag, n)            % Used in COBYLA
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(numel(Rdiag) >= min(m, n + 1) && numel(Rdiag) <= m, ...
                                 "MIN(M, N+1) <= SIZE(Rdiag) <= M", srname);
                debug_obj.assert(size(Q, 1) == m && size(Q, 2) == m, "SIZE(Q) == [M, M]", srname);
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-8, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                            fortran.power(consts_obj.TEN, min(12, consts_obj.MAXPOW10)) ...
                            * consts_obj.EPS * double(m + 1)));
                debug_obj.assert(linalg_obj.isorth(Q, 'tol', tol), ...
                                 "The columns of Q are orthonormal", srname); % Costly!
            ...
            cq(linalg_obj.trueloc(linalg_obj.isminor1(cq, cqa))) = ...
                consts_obj.ZERO; %%MATLAB: cq(isminor(cq, cqa)) = zero
            ...
            for k = m - 1:-1:n + 1
                if abs(cq(k + 1)) > 0
                    ...
                    cq(k) = ...
                        linalg_obj.hypotenuse(cq(k), ...
                                              cq(k + 1)); %cq(k) = sqrt(cq(k)**2 + cq(k + 1)**2)
            ...
            if n < m
                ...
                if abs(cq(n + 1)) > fortran.power(consts_obj.EPS, 2) ...
                   && ~linalg_obj.isminor0(cq(n + 1), cqa(n + 1))
                    n = n + 1;
                end
            end
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(n >= nsave && n <= min(nsave + 1, m), ...
                                 "NSAV <= N <= MIN(NSAV + 1, M)", srname);
                debug_obj.assert(numel(Rdiag) >= n && numel(Rdiag) <= m, ...
                                 "N <= SIZE(Rdiag) <= M", srname);
                debug_obj.assert(size(Q, 1) == m && size(Q, 2) == m, "SIZE(Q) == [M, M]", srname);
                debug_obj.assert(linalg_obj.isorth(Q, 'tol', tol), ...
                                 "The columns of Q are orthonormal", srname); % Costly!

                debug_obj.assert(all(abs(Q(:, 1:nsave) - Qsave(:, 1:nsave)) <= 0, 'all'), ...
                                 "Q(:, 1:NSAVE) is unchanged", srname);
                debug_obj.assert(all(abs(Rdiag(1:n - 1) - Rdsave(1:n - 1)) <= 0, 'all'), ...
                                 "Rdiag(1:N-1) is unchanged", srname);

                if n < m && infnan_obj.is_finite(linalg_obj.p_norm(c))
                    debug_obj.assert(linalg_obj.p_norm(linalg_obj.matprod12(c, Q(:, n + 1:m))) ...
                                     <= max(tol, tol * linalg_obj.p_norm(c)), ...
                                     "C^T*Q(:, N+1:M) == 0", srname);
                end
                if n >= 1
                    % N = 0 is possible.
                    debug_obj.assert(abs(linalg_obj.inprod(c, Q(:, n)) - Rdiag(n)) ...
                                     <= max(tol, tol * linalg_obj.inprod(abs(c), abs(Q(:, n)))) ...
                                     || ~infnan_obj.is_finite(Rdiag(n)), ...
        ...
        function [Q, R, n] = qradd_Rfull(~, c, Q, R, n)            % Used in LINCOA
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(size(R, 2) >= n + 1 && size(R, 2) <= m, ...
                                 "N+1 <= SIZE(R, 2) <= M", srname);
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-8, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                ...
                debug_obj.assert(all(linalg_obj.diag(R(:, 1:n)) > 0, 'all'), ...
                                 "DIAG(R(:, 1:N)) > 0", srname);
                Anew(:) = [reshape(linalg_obj.matprod22(Q, R(:, 1:n)), 1, []), c.'];
                Qsave = Q(:, 1:n); % For debugging only.
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(size(Q, 1) == m && size(Q, 2) == m, "SIZE(Q) = [M, M]", srname);
                debug_obj.assert(size(Q, 2) == size(R, 1), "SIZE(Q, 2) == SIZE(R, 1)", srname);
                debug_obj.assert(size(R, 2) >= n && size(R, 2) <= m, "N <= SIZE(R, 2) <= M", ...
                                 srname);
                debug_obj.assert(linalg_obj.isorth(Q, 'tol', tol), ...
                                 "The columns of Q are orthogonal", srname);
                debug_obj.assert(linalg_obj.istriu(R), "R is upper triangular", srname);
                ...
                debug_obj.assert(all(abs(Q(:, 1:n - 1) - Qsave(:, 1:n - 1)) <= 0, 'all'), ...
                                 "Q(:, 1:N-1) is unchanged", srname);
                debug_obj.assert(all(abs(R(:, 1:n - 1) - Rsave(:, 1:n - 1)) <= 0, 'all'), ...
                                 "R(:, 1:N-1) is unchanged", srname);

                % The following test may fail.
                debug_obj.assert(all(abs(Anew - linalg_obj.matprod22(Q, R(:, 1:n))) ...
                                     <= max(tol, tol * max(abs(Anew), [], 'all')), 'all'), ...
                                 "Anew = Q*R", srname);
            end
        end
        function [Q, Rdiag] = qrexc_Rdiag(~, A, Q, Rdiag, i)            % Used in COBYLA
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(size(Q, 1) == m && size(Q, 2) >= n && size(Q, 2) <= m, ...
            ...
            Rdiag(n) = ...
                linalg_obj.inprod(Q(:, n), ...
                                  A(:, ...
                                    i)); % Calculate RDIAG(N) from scratch. See the comments above.
            ...
            if consts_obj.DEBUGGING
                ...
                Qsave(:, i:n) = Q(:, i:n);
                debug_obj.assert(all(abs(Q - Qsave) <= 0, 'all'), ...
                                 "Q is unchanged except Q(:, I:N)", srname);
                debug_obj.assert(all(abs(Rdiag(1:i - 1) - Rdsave(1:i - 1)) <= 0, 'all'), ...
                                 "Rdiag(1:I-1) is unchanged", srname);

                Anew(:) = [reshape(A(:, 1:i - 1), 1, []), reshape(A(:, i + 1:n), 1, []), A(:, i).'];
                QtAnew(:) = linalg_obj.matprod22(Q.', Anew);
                debug_obj.assert(linalg_obj.istriu(QtAnew, 'tol', tol), ...
                                 "Q^T*Anew is upper triangular", srname);
                % The following test may fail if RDIAG is not calculated from scratch.
                debug_obj.assert(linalg_obj.p_norm(linalg_obj.diag(QtAnew) - Rdiag) ...
                                 <= max(tol, ...
                                        tol ...
                                        * linalg_obj.p_norm(arrayfun(@(k) linalg_obj.inprod(abs(Q(:, ...
                                                                                                  k)), ...
                                                                                            abs(Anew(:, ...
                                                                                                     k))), ...
                                                                     (1:n)'))), ...
                                 "Rdiag == diag(Q^T*Anew)", srname);
        ...
        function [Q, R] = qrexc_Rfull(~, Q, R, i)            % Used in LINCOA
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(i >= 1 && i <= n, "1 <= I <= N", srname);
                debug_obj.assert(size(Q, 2) == size(R, 1), "SIZE(Q, 2) == SIZE(R, 1)", srname);
                debug_obj.assert(size(Q, 2) >= n && size(Q, 2) <= m, "N <= SIZE(Q, 2) <= M", ...
                                 srname);
                debug_obj.assert(size(R, 1) >= n && size(R, 1) <= m, "N <= SIZE(R, 1) <= M", ...
                                 srname);
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-8, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                ...
                Anew = ...
                    reshape([reshape(Anew(:, 1:i - 1), 1, []), reshape(Anew(:, i + 1:n), 1, []), ...
                             Anew(:, i).'], size(Anew));
                Qsave = Q; % For debugging only.
            ...
            for k = i:n - 1
                ...
                hypt = ...
                    linalg_obj.hypotenuse(R(k + 1, k + 1), ...
                                          R(k, ...
                                            k ...
                                            + 1)); %hypt = sqrt(R(k, k + 1)**2 + R(k + 1, k + 1)**2)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(all(abs(Q - Qsave) <= 0, 'all'), ...
                                 "Q is unchanged except Q(:, I:N)", srname);
                debug_obj.assert(all(abs(R(:, 1:i - 1) - Rsave(:, 1:i - 1)) <= 0, 'all'), ...
                                 "R(:, 1:I-1) is unchanged", srname);

                % The following test may fail.
                debug_obj.assert(all(abs(Anew - linalg_obj.matprod22(Q, R)) ...
                                     <= max(tol, tol * max(abs(Anew), [], 'all')), 'all'), ...
                                 "Anew = Q*R", srname);
            end
        ...
        function qinc = quadinc_d0(~, d, xpt, gq, pq, varargin)
            ...
            if ismember('hq', ipObj.UsingDefaults)
                qinc = ...
                    linalg_obj.inprod(d, gq) ...
                    + consts_obj.HALF * linalg_obj.inprod(dxpt, pq .* dxpt);
            else
                qinc = ...
                    linalg_obj.inprod(d, gq + consts_obj.HALF * linalg_obj.matprod21(hq, d)) ...
                    + consts_obj.HALF * linalg_obj.inprod(dxpt, pq .* dxpt);
            end
            %%MATLAB:
            %%if nargin >= 5
        ...
        function err = errquad(obj, fval, xpt, gq, pq, hq, varargin)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(~any(infnan_obj.is_nan_sp(fval) | infnan_obj.is_posinf(fval), ...
                                      'all'), "FVAL is not NaN/+Inf", srname);
                debug_obj.assert(all(infnan_obj.is_finite(xpt), 'all'), "XPT is finite", srname);
                debug_obj.assert(numel(gq) == n, "SIZE(GQ) == N", srname);
                debug_obj.assert(numel(pq) == npt, "SIZE(PQ) == NPT", srname);
                debug_obj.assert(size(hq, 1) == n && linalg_obj.issymmetric(hq), ...
            ...
            if all(infnan_obj.is_finite(qval), 'all')
                fmq = fval - qval;
                err = (max(fmq) - min(fmq)) / max([consts_obj.ONE; abs(fval)], [], 'all');
            else
        ...
        function y = omega_col(~, idz, zmat, k)
            ...
            % Preconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(idz >= 1 && idz <= size(zmat, 2) + 1, ...
                                 "1 <= IDZ <= SIZE(ZMAT, 2) + 1", srname);
        ...
        function err = errh(~, idz, bmat, zmat, xpt)
            ...
            Omega(:) = ...
                -linalg_obj.matprod22(zmat(:, 1:idz - 1), zmat(:, 1:idz - 1).') ...
                + linalg_obj.matprod22(zmat(:, idz:npt - n - 1), zmat(:, idz:npt - n - 1).');
            maxabs = ...
                max([consts_obj.ONE, max(abs(A), [], 'all'), max(abs(Omega), [], 'all'), ...
                     max(abs(bmat), [], 'all')]);
            U(:) = ...
                linalg_obj.eye1(npt) - linalg_obj.matprod22(A, Omega) ...
                - linalg_obj.matprod22(xpt.', bmat(:, 1:npt));
            V(:) = ...
                -linalg_obj.matprod22(bmat(:, 1:npt), A) ...
                - linalg_obj.matprod22(bmat(:, npt + 1:npt + n), xpt);
            ...
            e(2, 1) = max(abs(fortran.sum(Omega, 1)));
            e(2, 2) = abs(fortran.sum(r, 'all') - consts_obj.ONE);
            e(2, 3) = max(abs(fortran.sum(bmat(:, 1:npt), 2)));
            e(3, 1) = max(abs(linalg_obj.matprod22(xpt, Omega)), [], 'all');
            e(3, 2) = max(abs(linalg_obj.matprod21(xpt, r)));
            e(3, 3) = ...
                max(abs(linalg_obj.matprod22(xpt, bmat(:, 1:npt).') - linalg_obj.eye1(n)), [], ...
                    'all');
        ...
        function [idz, bmat, zmat, info] = updateh(obj, knew, kref, d, xpt, idz, bmat, zmat)
            ...
            if ~(infnan_obj.is_finite(fortran.sum(abs(hcol), 'all') ...
                                      + fortran.sum(abs(vlag), 'all') + abs(beta)) ...
                 && abs(denom) > 0)
                if nargout >= 4
            ...
            if jl == 1
                ...
                scala = ...
                    consts_obj.ONE ...
                    / fortran.sqrt(abs(beta) * fortran.power(temp, 2) ...
                                   + fortran.power(tau, ...
                                                   2)); % 1/SQRT(ZETA) in (4.19)-(4.20) of NEWUOA paper
                scalb = scala * sqrtdn;
        ...
        function vlag = calvlag_lfqint(obj, kref, bmat, d, xpt, zmat, varargin)
            ...
            if consts_obj.DEBUGGING
                debug_obj.assert(n >= 1 && npt >= n + 2, "N >= 1, NPT >= N + 2", srname);
                debug_obj.assert(idz_loc >= 1 && idz_loc <= size(zmat, 2) + 1, ...
                                 "1 <= ID <= SIZE(ZMAT, 2) + 1", srname);
                debug_obj.assert(kref >= 1 && kref <= npt, "1 <= KREF <= NPT", srname);
                debug_obj.assert(size(bmat, 1) == n && size(bmat, 2) == npt + n, ...
                                 "SIZE(BMAT)==[N, NPT+N]", srname);
                debug_obj.assert(linalg_obj.issymmetric(bmat(:, npt + 1:npt + n)), ...
            ...
            vlag(1:npt) = ...
                obj.omega_mul(idz_loc, zmat, wcheck) + linalg_obj.matprod12(d, bmat(:, 1:npt));
            ...
            if consts_obj.DEBUGGING
                ...
                tol = ...
                    max(fortran.power(consts_obj.TEN, max(-8, -consts_obj.MAXPOW10)), ...
                        min(0.1, ...
                            fortran.power(consts_obj.TEN, min(12, consts_obj.MAXPOW10)) ...
                            * consts_obj.EPS * double(npt + n)));
                debug_obj.wassert(abs(fortran.sum(vlag(1:npt), 'all') - consts_obj.ONE) ...
                                  / double(npt) <= tol ...
                                  || floor(-log10(eps('double'))) ...
                                     < floor(-log10(eps('double'))), "SUM(VLAG(1:NPT)) == 1", ...
                                  srname);
            end
        ...
        function beta = calbeta(obj, kref, bmat, d, xpt, zmat, varargin)
            ...
            beta = ...
                fortran.power(dxref, 2) + dsq * (xrefsq + dxref + dxref + consts_obj.HALF * dsq) ...
                - dvlag - wvlag;
            %---------------------------------------------------------------------------------------------------%
        ...
        function den = calden(obj, kref, bmat, d, xpt, zmat, varargin)
            ...
            hdiag = ...
                -fortran.sum(fortran.power(zmat(:, 1:idz_loc - 1), 2), 2) ...
                + fortran.sum(fortran.power(zmat(:, idz_loc:size(zmat, 2)), 2), 2);
        ...
        function vlag = calvlag_qint(~, pl, d, xref, kref)
            ...
            if consts_obj.DEBUGGING
                debug_obj.assert(numel(d) == n, "SIZE(D) = N", srname);
                debug_obj.assert(size(pl, 2) == (n + 1) * (n + 2) / 2 ...
                                 && size(pl, 1) == size(pl, 2) - 1, ...
        ...
        function ij = setij(~, n, npt, varargin)
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(ij, 1) == 2 && size(ij, 2) == max(0, npt - 2 * n - 1), ...
                                 "SIZE(IJ) == [2, NPT - 2*N - 1]", srname);
                debug_obj.assert(all(ij >= 1 & ij <= n, 'all'), "1 <= IJ <= N", srname);
                if ismember('sorting_direction', ipObj.UsingDefaults)
                    debug_obj.assert(all(ij(1, :).' ~= ij(2, :).', 'all'), ...
                                     "IJ(1, :) /= IJ(2, :)", srname);
                else
