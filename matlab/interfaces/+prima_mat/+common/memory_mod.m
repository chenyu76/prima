classdef memory_mod
    ...
    methods
            ...
            elseif numel(varargin) == 2 ...
                   && (isinteger(varargin{1}) ...
                       || isnumeric(varargin{1}) ...
                          && (isreal(varargin{1}) ...
                              && all(fix(varargin{1}) == varargin{1}, 'all'))) ...
                   && isvector(varargin{1})
            ...
            elseif numel(varargin) == 3 ...
                   && (isinteger(varargin{1}) ...
                       || isnumeric(varargin{1}) ...
                          && (isreal(varargin{1}) ...
                              && all(fix(varargin{1}) == varargin{1}, 'all'))) ...
                   && (~isvector(varargin{1}) && ~isscalar(varargin{1}))
                [varargout{1:nargout}] = obj.alloc_imatrix(varargin{:});
            elseif numel(varargin) == 3 && isfloat(varargin{1}) ...
                   && (~isvector(varargin{1}) && ~isscalar(varargin{1}))
        ...
        function x = alloc_rvector_sp(~, n)
            ...
            debug_obj.validate(alloc_status == 0, ...
                               "Memory allocation succeeds (ALLOC_STATUS == 0)", srname);
            debug_obj.validate(~isequal(x, []), "X is allocated", srname);
        ...
        function x = alloc_rmatrix_sp(~, m, n)
            ...
            % Allocate memory for X. Initialize X to a compiler-independent strange value.
            x = NaN(m, n);
            alloc_status = 0; % Absoft does not support the SOURCE keyword as of 2022.
            x(:) = -realmax; % Costly if X is of a large size.
