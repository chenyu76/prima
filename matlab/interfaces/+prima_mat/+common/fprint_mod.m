        ...
        function fprint(~, string, varargin)
            ...
            if consts_obj.DEBUGGING
                if ~ismember('funit', ipObj.UsingDefaults)
                    debug_obj.assert(funit ~= consts_obj.STDIN, "The file unit is not STDIN", ...
                                     srname);
                    if ~ismember('fname', ipObj.UsingDefaults)
                        debug_obj.assert((strlength(fname) == 0) ...
                                         == (funit == consts_obj.STDOUT ...
                                             || funit == consts_obj.STDERR), ...
                                         "The file name is empty if and only if the file unit is either STDOUT or STDERR", ...
                                         srname);
                    end
                end
                if ~ismember('faction', ipObj.UsingDefaults)
                    debug_obj.assert(faction == "write" || faction == "w" || faction == "append" ...
                                     || faction == "a", ...
            ...
            if consts_obj.DEBUGGING
                debug_obj.assert((strlength(fname_loc) == 0) ...
                                 == (funit_loc == consts_obj.STDOUT ...
                                     || funit_loc == consts_obj.STDERR), ...
                                 "The file name is empty if and only if the file unit is either STDOUT or STDERR", ...
                                 srname);
            end
