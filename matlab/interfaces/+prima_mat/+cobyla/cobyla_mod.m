    ...
    methods
        function [x, f_loc, cstrv_loc, nlconstr, nf_loc, xhist, fhist, chist, nlchist, info_loc] = cobyla(obj, calcfc, m_nlcon, x, varargin)
            ...
            if consts_obj.DEBUGGING
                ...
                if nargout >= 4
                    debug_obj.assert(numel(nlconstr) == m_nlcon, "SIZE(NLCONSTR) == M_NLCON", srname);
                end
                ...
                if ~ismember('nlconstr0', ipObj.UsingDefaults)
                    debug_obj.assert(~ismember('f0', ipObj.UsingDefaults), "If NLCONSTR0 is present, then F0 is present", srname);
                end
                if ~ismember('f0', ipObj.UsingDefaults)
                    debug_obj.assert(infnan_obj.is_nan_sp(f0) || ~ismember('nlconstr0', ipObj.UsingDefaults), "If F0 is present and not NaN, then NLCONSTR0 is present", srname);
                end
            end
            ...
            if ~ismember('nlconstr0', ipObj.UsingDefaults)
                if numel(nlconstr0) ~= m_nlcon
                    if consts_obj.DEBUGGING
                        debug_obj.errstop(srname, "SIZE(NLCONSTR0) /= M_NLCON. Exiting", 'code', infos_obj.INVALID_INPUT);
                    else
            ...
            constr_loc(1:m - m_nlcon) = evaluate_obj.moderatec(linalg_obj.matprod12(x, amat) - bvec); % Linear and bound constraints
            ...
            if ~ismember('f0', ipObj.UsingDefaults) && ~ismember('nlconstr0', ipObj.UsingDefaults) && all(infnan_obj.is_finite(x), 'all')
            ...
            cstrv_loc = linalg_obj.maximum([consts_obj.ZERO; reshape(constr_loc, [], 1)]);
        ...
        function [amat, bvec] = get_lincon(~, Aeq, Aineq, beq, bineq, xl, xu, amat, bvec)
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(amat, 1) == numel(xl) && size(amat, 2) == numel(bvec), "SIZE(AMAT) == [SIZE(X), SIZE(BVEC)]", srname);
            end
        end
