classdef cobyla_mod
    ...
    methods
        function [x, f_loc, cstrv_loc, nlconstr, nf_loc, xhist, fhist, chist, nlchist, ...
            ...
            nlchist = []; % NLCHIST(M_NLCON, MAXCONHIST)
            ...
            if consts_obj.DEBUGGING
                ...
                if ~ismember('nlconstr0', ipObj.UsingDefaults)
                    debug_obj.assert(~ismember('f0', ipObj.UsingDefaults), ...
                                     "If NLCONSTR0 is present, then F0 is present", srname);
                end
                if ~ismember('f0', ipObj.UsingDefaults)
                    debug_obj.assert(infnan_obj.is_nan_sp(f0) ...
                                     || ~ismember('nlconstr0', ipObj.UsingDefaults), ...
                                     "If F0 is present and not NaN, then NLCONSTR0 is present", ...
                                     srname);
                end
            end
            ...
            if ~ismember('nlconstr0', ipObj.UsingDefaults)
                if numel(nlconstr0) ~= m_nlcon
                    if consts_obj.DEBUGGING
                        debug_obj.errstop(srname, "SIZE(NLCONSTR0) /= M_NLCON. Exiting", ...
                                          'code', infos_obj.INVALID_INPUT);
                    else
            ...
            constr_loc(1:m - m_nlcon) = ...
                evaluate_obj.moderatec(linalg_obj.matprod12(x, amat) ...
                                       - bvec); % Linear and bound constraints
            ...
            if ~ismember('f0', ipObj.UsingDefaults) ...
               && ~ismember('nlconstr0', ipObj.UsingDefaults) && all(infnan_obj.is_finite(x), 'all')
            ...
            linalg_obj.maximum([consts_obj.ZERO; constr_loc]);
            ...
            if nargout >= 9
                ...
                %---------------------------------------------------------------%
                memory_obj.alloc_rmatrix_sp(m_nlcon, nhist); % Removable in F2003.
                %---------------------------------------------------------------%
                nlchist = ...
            ...
            if consts_obj.DEBUGGING
                ...
                if nargout >= 8
                    ...
                    debug_obj.assert(~any(chist < 0 | infnan_obj.is_nan_sp(chist) ...
                                     ...
                                     "CHIST does not contain nonnegative values or NaN/+Inf", ...
                                     srname);
                end
                if nargout >= 9
                    debug_obj.assert(size(nlchist, 1) == m_nlcon && size(nlchist, 2) == nhist, ...
                                     "SIZE(NLCHIST) == [M_NLCON, NHIST]", srname);
                    debug_obj.assert(~any(infnan_obj.is_nan_sp(nlchist) ...
                                          | infnan_obj.is_posinf(nlchist), 'all'), ...
                                     "NLCHIST does not contain NaN/+Inf", srname);
                end
        ...
        function [amat, bvec] = get_lincon(~, Aeq, Aineq, beq, bineq, xl, xu)
            ...
            memory_obj.alloc_rvector_sp(m_lcon);
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(size(amat, 1) == numel(xl) && size(amat, 2) == numel(bvec), ...
