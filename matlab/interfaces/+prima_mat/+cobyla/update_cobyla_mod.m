    ...
    methods
        function [conmat, cval, fval, sim, simi, info] = ...
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(~(infnan_obj.is_nan_sp(cstrv) || infnan_obj.is_posinf(cstrv)), ...
                                 "CSTRV is not NaN/+Inf", srname);
            ...
            if jdrop <= n
                ...
                simi = ...
                    simi ...
                    + linalg_obj.outprod(simid, ...
                                         sum_simi ./ (consts_obj.ONE - fortran.sum(simid, 'all')));
            end
            ...
            erri = ...
                linalg_obj.maximum2(abs(linalg_obj.matprod22(simi, sim(:, 1:n)) ...
                                        - linalg_obj.eye1(n))); % MAXIMUM(X) returns NaN if X contains NaN
            if erri > consts_obj.TENTH * itol || infnan_obj.is_nan_sp(erri)
                simi_test(:) = linalg_obj.inv(sim(:, 1:n));
                erri_test = ...
                    linalg_obj.maximum2(abs(linalg_obj.matprod22(simi_test, sim(:, 1:n)) ...
                                            - linalg_obj.eye1(n)));
                if erri_test < erri ...
                   || infnan_obj.is_nan_sp(erri) && ~infnan_obj.is_nan_sp(erri_test)
                    simi = simi_test;
                    erri = erri_test;
                end
            end
        ...
        function [conmat, cval, fval, sim, simi, info] = ...
            ...
            % Postconditions
            if consts_obj.DEBUGGING
                debug_obj.assert(obj.findpole(cpen, cval, fval) == n + 1 ...
                                 || info == infos_obj.DAMAGING_ROUNDING, ...
                                 "The best point is SIM(:, N+1) unless the rounding is damaging", ...
                                 srname);
                debug_obj.assert(size(conmat, 1) == m && size(conmat, 2) == n + 1, ...
                                 "SIZE(CONMAT) = [M, N+1]", srname);
                debug_obj.assert(~any(infnan_obj.is_nan_sp(conmat) ...
        ...
        function jopt = findpole(~, cpen, cval, fval)
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(jopt == n + 1 || phi(jopt) < phi(n + 1) ...
                                 || phi(jopt) <= phi(n + 1) && cval(jopt) < cval(n + 1), ...
                                 "JOPT = N+1 unless PHI(JOPT) < PHI(N+1) or PHI(JOPT) <= PHI(N+1) and CVAL(JOPT) < CVAL(N+1)", ...
                                 srname);
            end
        end
