classdef geometry_lincoa_mod
    ...
    methods
        function knew = setdrop_tr(~, idz, kopt, ximproved, bmat, d, delta, rho, xpt, zmat)
            ...
            weight = ...
                fortran.power(max(consts_obj.ONE, ...
                                  distsq ...
                                  ./ fortran.power(max(consts_obj.TENTH * delta, rho), 2)), ...
                              3); % Powell's NEWUOA code
        ...
        function [feasible, s] = ...
            ...
            vlagabs(knew) = ...
                abs(stplen(knew) * dderiv(knew)) ...
                + fortran.power(stplen(knew), 2) * abs(dderiv(knew) - consts_obj.ONE);
            ...
            cstrv = ...
                linalg_obj.maximum1([consts_obj.ZERO
                                     linalg_obj.matprod12(s, ...
                                                          amat(:, ...
                                                               linalg_obj.trueloc(rstat >= 0))) ...
                                     - rescon(linalg_obj.trueloc(rstat >= 0))]);
            feasible = cstrv <= 0;
            ...
            pglag(:) = ...
                linalg_obj.matprod21(qfac(:, nact + 1:n), ...
                                     linalg_obj.matprod12(glag, qfac(:, nact + 1:n)));
            ...
            if nact > 0 && gnorm > consts_obj.EPS && infnan_obj.is_finite(gnorm)
                ...
                cstrv = ...
                    linalg_obj.maximum1([consts_obj.ZERO
                                         linalg_obj.matprod12(pgstp, ...
                                                              amat(:, ...
                                                                   linalg_obj.trueloc(rstat ...
                                                                                      == 1))) ...
                                         - rescon(linalg_obj.trueloc(rstat == 1))]);
                ...
                cvtol = ...
                    max(consts_obj.EPS * linalg_obj.p_norm(pgstp), ...
                        consts_obj.TEN ...
                        * linalg_obj.named_norm_vec(linalg_obj.matprod12(pgstp, ...
                                                                         amat(:, iact(1:nact))), ...
                                                    "inf"));
                take_pgstp = false;
                if cstrv <= cvtol
            ...
            if consts_obj.DEBUGGING
                ...
                debug_obj.assert(linalg_obj.p_norm(s) > consts_obj.HALF * delbar ...
                                 && linalg_obj.p_norm(s) < consts_obj.TWO * delbar, ...
                                 "DELBAR/2 < ||S|| < 2*DELBAR", srname);
            end
