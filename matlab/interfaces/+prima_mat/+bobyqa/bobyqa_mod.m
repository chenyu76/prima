        ...
        function [x, f_loc, nf_loc, xhist, fhist, info] = bobyqa(~, calfun, x, varargin)
            ...
            if any(xu_loc - xl_loc < consts_obj.TWO * consts_obj.EPS, 'all')
                ...
                debug_obj.warning(solver, ...
                                  "There is no space between the lower and upper bounds of variable " ...
                                  + string_obj.int2str(min(linalg_obj.trueloc(xu_loc - xl_loc ...
                                                                              < consts_obj.TWO ...
                                                                                * consts_obj.EPS))) ...
            ...
            [iprint_loc, maxfun_loc, maxhist_loc, ftarget_loc, rhobeg_loc, rhoend_loc, npt_loc, ...
             ~, ~, ~, eta1_loc, eta2_loc, gamma1_loc, gamma2_loc, x] = ...
                preproc_obj.preproc(solver, n, iprint_loc, maxfun_loc, maxhist_loc, ftarget_loc, ...
                                    ...
                                    'gamma2', gamma2_loc, 'has_rhobeg', has_rhobeg, ...
                                    'honour_x0', honour_x0_loc, 'xl', xl_loc, 'xu', xu_loc, ...
                                    'x0', x);
            ...
            if ismember('callback_fcn', ipObj.UsingDefaults)
                [x, nf_loc, f_loc, fhist_loc, xhist_loc, info_loc] = ...
                    bobyqb_obj.bobyqb(calfun, iprint_loc, maxfun_loc, npt_loc, eta1_loc, ...
                                      eta2_loc, ftarget_loc, gamma1_loc, gamma2_loc, rhobeg_loc, ...
                                      rhoend_loc, xl_loc, xu_loc, x, fhist_loc, xhist_loc);
            else
                [x, nf_loc, f_loc, fhist_loc, xhist_loc, info_loc] = ...
                    bobyqb_obj.bobyqb(calfun, iprint_loc, maxfun_loc, npt_loc, eta1_loc, ...
                                      eta2_loc, ftarget_loc, gamma1_loc, gamma2_loc, rhobeg_loc, ...
            ...
            if consts_obj.DEBUGGING
                ...
                if ~ismember('xl', ipObj.UsingDefaults)
                    if numel(xl) == numel(x)
                        ...
                        if nargout >= 4
                            for k = 1:nhist
                                debug_obj.assert(all(xhist(:, k) >= xl, 'all'), "XHIST >= XL", ...
                                                 srname);
                            end
                        end
                    end
                ...
                if ~ismember('xu', ipObj.UsingDefaults)
                    if numel(xu) == numel(x)
                        ...
                        if nargout >= 4
                            for k = 1:nhist
                                debug_obj.assert(all(xhist(:, k) <= xu, 'all'), "XHIST <= XU", ...
                                                 srname);
                            end
                        end
                    end
                ...
                if nargout >= 5
                    ...
                    debug_obj.assert(~any(fhist < f_loc, 'all'), "F is the smallest in FHIST", ...
                                     srname);
                end
            end
