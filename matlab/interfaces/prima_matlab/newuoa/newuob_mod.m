classdef newuob_mod
    ...
    methods
        function [x, nf, f, fhist, xhist, info] = newuob(~, calfun, iprint, maxfun, npt, eta1, eta2, ftarget, gamma1, gamma2, rhobeg, rhoend, x, fhist, xhist, varargin)
            ...
            geometry_newuoa_obj = geometry_newuoa_mod();
            initialize_newuoa_obj = initialize_newuoa_mod();
            trustregion_newuoa_obj = trustregion_newuoa_mod();
            update_newuoa_obj = update_newuoa_mod();
            ...
            for tr = 1:maxtr
                ...
                if improve_geo
                    ...
                    delbar = max(min(consts_obj.TENTH * fortran.sqrt(max(distsq, [], 'all')), consts_obj.HALF * delta), rho); % Powell's code
