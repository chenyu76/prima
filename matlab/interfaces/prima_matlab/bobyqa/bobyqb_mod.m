...
classdef bobyqb_mod
    ...
    methods
        function [x, nf, f, fhist, xhist, info] = bobyqb(obj, calfun, iprint, maxfun, npt, eta1, eta2, ftarget, gamma1, gamma2, rhobeg, rhoend, xl, xu, x, fhist, xhist, varargin)
            ...
            debug_obj = debug_mod(); %, wassert, validate
            ...
            powalg_obj = powalg_mod(); %, errquad
            ratio_obj = ratio_mod();
            redrho_obj = redrho_mod();
            shiftbase_obj = shiftbase_mod();
            xinbd_obj = xinbd_mod();

            % Solver-specific modules
            geometry_bobyqa_obj = geometry_bobyqa_mod();
            initialize_bobyqa_obj = initialize_bobyqa_mod();
            rescue_obj = rescue_mod();
            trustregion_bobyqa_obj = trustregion_bobyqa_mod();
            update_bobyqa_obj = update_bobyqa_mod();
            ...
            for tr = 1:maxtr
                ...
                if shortd || trfail
                    ...
                    to_rescue = (ximproved && ~(infnan_obj.is_finite(sum(abs(vlag), 'all')) && any(den > max(fortran.dot_power(vlag(1:npt), 2), [], 'all'), 'all')));
                ...
                if improve_geo
                    ...
                    delbar = max(min(consts_obj.TENTH * fortran.sqrt(max(distsq, [], 'all')), delta), rho); % Powell's code
        ...
        function ebound = errbd(~, crvmin, d, gopt, hq, moderr_rec, pq, rho, sl, su, xopt, xpt)
            ...
            bsecond(:) = consts_obj.HALF * (linalg_obj.diag(hq) + linalg_obj.matprod21(fortran.dot_power(xpt, 2), pq)) * fortran.power(rho, 2);
