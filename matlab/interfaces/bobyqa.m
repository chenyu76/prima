...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:2), cell(1, 4), varargin(3:4), {[]}, varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
...
try % preprima is a private function that may generate public errors; error-handling needed
    [fun, x0, ~, ~, ~, ~, lb, ub, ~, options, probinfo] = preprima(args{:});
catch exception
...
elseif ~strcmp(invoker, 'prima') && probinfo.feasibility_problem
    ...
    output.x = x0;  % preprima has set x0 to a feasible point
